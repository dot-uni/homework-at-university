...
template <typename T> class Stack 
{
    ...
    public:
        ...
        Stack(const T& data) : head_(new Node_Stack<T>{data, nullptr}), size_(1) {}
        Stack(const T* data, size_t n) {
            if (n <= 0) throw std::runtime_error("Размер стека должен быть натуральным числом.");
            size_ = n;
            head_ = new Node_Stack<T>{data[0], nullptr};
            for (size_t i = 1; i < n; ++i) {
                Node_Stack<T>* tmp = new Node_Stack<T>(data[i], head_);
                head_ = tmp;
            } 
        }
...
void Stack<T>::push(const T& obj) {
    Node_Stack<T>* new_node = new Node_Stack<T>{obj, head_};
    head_ = new_node;
    size_++;
}
...
T Stack<T>::pop() {
    ...
    Node_Stack<T>* new_head = head_->get_next();
    delete head_;
    head_ = new_head;
    return data; 
} 
